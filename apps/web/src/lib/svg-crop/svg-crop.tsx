...
export class ReactCrop extends PureComponent<ReactCropProps, ReactCropState> {
  ...
  onComponentKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const ctrlCmdPressed = /Mac/.exec(navigator.platform)
      ? e.metaKey
      : e.ctrlKey;
  ...
  resizeCrop() {
    ...
    if (aspect || ReactCrop.xyOrds.includes(ord)) {
      nextCrop = containedCrop;
    } else if (ReactCrop.xOrds.includes(ord)) {
      nextCrop.x = containedCrop.x;
      nextCrop.width = containedCrop.width;
    } else if (ReactCrop.yOrds.includes(ord)) {
  ...
  render() {
    ...
    return (
      <div className={componentClasses} ref={this.componentRef} style={style}>
        <div
        ...
        {crop ? (
          <svg className="ReactCrop__crop-mask" height="100%" width="100%">
            <defs>
